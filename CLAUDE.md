...
| Sales | Retours (restock on approval only; frontend `/retours` added 2026-08-17 — list+stats, facture-scoped create, approve/cancel with stock-effect confirms), Credit-limit enforcement, Pricing (no TVA by design) | ✅ |
...
| Inventory | Codes-barres (`produits.code_barre`, existing column from `007`): writable via product create/update, exact-match in product search (`getAll`, `searchFuzzy`, magasin picker), scan auto-adds a line in NouvelleFacture/NouveauDevis (2026-08-17) | ✅ |
...
| Accounting | General Ledger, Comptabilité, Compensation, Caisse magasin/hierarchy, Dépenses (V2 — liste filtrable search/catégorie/période, 2026-08-17), Reporting | ✅ |
...
| Admin/Infra | Auth, Admin users/allocation, Tiers, Clients, CRM, Company settings, Audit log | ✅ |
| Sales | Relances clients (`/relances`, 2026-08-17) — file de recouvrement bâtie sur la balance âgée; chaque relance est une interaction CRM `type='relance'` (pas de nouvelle table). `getReceivablesAging` renvoie désormais telephone/email/plus_ancienne_facture | ✅ |
| Admin/Infra | Notifications SSE — emitters wired 2026-08-17 (facture créée, paiement encaissé, stock bas post-vente), all fired **after COMMIT** via `NotificationService.safely()`; delivery is **role-scoped** (`broadcast(..., { roles })`) so amounts/client names reach admin/manager only. Persistent notification centre (history, read/unread) still missing — the bell only shows réappro demandes | 🟡 |
