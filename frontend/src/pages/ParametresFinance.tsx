            ...
            <TableBody>
              {cotisations.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell>
                    <input type="checkbox" checked={c.actif} onChange={(e) => setCot(c.id, { actif: e.target.checked })} />
                  </TableCell>
                  <TableCell>
                    <Button size="icon" variant="ghost" disabled={savingC === c.id} onClick={() => saveCotisation(c)}>
                      {savingC === c.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
        ...
        <CardContent className="overflow-x-auto">
                ...
                <TableRow key={i}>
                  <TableCell>
                    <Input type="number" className="h-8" value={b.tranche_min as any}
                      onChange={(e) => setBar(i, { tranche_min: e.target.value })} />
                  </TableCell>
                  <TableCell>
                    <Input type="number" className="h-8" value={(b.tranche_max ?? '') as any}
                      onChange={(e) => setBar(i, { tranche_max: e.target.value === '' ? null : e.target.value })} placeholder="∞" />
                  </TableCell>
                  <TableCell>
                    <Input type="number" step="0.01" className="h-8" value={toPct(b.taux)}
                      onChange={(e) => setBar(i, { taux: fromPct(e.target.value) })} />
                  </TableCell>
                  <TableCell>
                    <Button size="icon" variant="ghost" onClick={() => delBar(i)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          ...
          <div className="flex justify-end pt-3">
            <Button onClick={saveBaremes} disabled={savingB} className="gap-2">
              {savingB ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Enregistrer le barème
            </Button>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={match.bloquer} onChange={(e) => setMatch({ ...match, bloquer: e.target.checked })} />
            ...
            <div className="flex justify-end">
              <Button onClick={saveMatch} disabled={savingM} className="gap-2">
                {savingM ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Enregistrer
              </Button>
            </div>
          </CardContent>
        </Card>
