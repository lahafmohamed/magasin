...
> **✅ Sprint 2 landed 2026-07-20** — commits `02fd263` (auth cluster: deactivated-user lockout + `actif` check in authenticate, session revocation on deactivate/password-change, admin password policy via Zod, session TTL derived from JWT_EXPIRATION), `aa17507` (route hardening: attachment entity ACL, patch-router tiersId/payslipId, log token redaction, dev-script prod guards), `510c04b` (Zod on money/stock routes: caisse transfer, paiement update, stock-transfer, stock-locations, caisse create), `b976e60` (GL/caisse-hierarchy/supplier-payment business errors → 4xx), `cbe4879` (FE: DocumentListPage error branch, 4 orphan routes wired, Inventaire double-submit guard), `f891b36` (audit-log on payment/GL/pièce/user-admin paths). **P1-remainder pass (also 2026-07-20):** `bc6358a` (migration 091: retour/commande line-qty CHECKs + period-lock extended to UPDATE/DELETE), `84e8d16` (EXPORT_MAX_ROWS cap on /factures + GL exports, LIMIT on getDepotStockForDemande + clients/:id/historique), `85f34ad` (auth login/change-password/register + retours statut + avoirs apply validateBody, password policy strengthened). **279 backend + 24 FE tests green on dev and a from-scratch CI DB.** Double-write on `produits.stock` **verified NOT a bug** (see below). **Still open in P1:** error-mapping done for Return/Reception/POS (`61cd9d2`); remaining `err.message`/400-catch-all controllers = Demande/DepenseV2/CaisseMagasin/Payroll (each needs its service's plain `throw new Error` → `businessError`, then `businessStatusOf` in the controller); remaining validateBody on demandes/payroll/user-location-assignments.
...
- ~~**Double-write on `produits.stock`**~~ — **verified NOT a bug (2026-07-20).** All 6 `UPDATE produits SET stock` sites (FactureService, BonLivraisonService, ReturnService) sit inside a `useLegacyStock` / `!splUpdated` **legacy-fallback branch**, mutually exclusive with the `stock_par_location` update that fires the `020` sync trigger. They only run for products with no `stock_par_location` row, where `produits.stock` is the authoritative store (not a cache). Removing them would break stock tracking for legacy products. No change.
