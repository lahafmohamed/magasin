// Wipe transactional data in database, keep system ref tables.
// Seed admin user + depot principal + magasin.
import pg from 'pg';
...
async function main() {
  ...
  try {
    ...
    const mag = await client.query(
      `INSERT INTO stock_locations (code, nom, location_type, est_principal, actif)
       VALUES ('MAG_TREICHVILLE', 'Magasin Treichville', 'magasin', false, true)
       RETURNING id`
    );
    ...
    await client.query(
      `INSERT INTO magasins (location_id, code, nom, actif)
       VALUES ($1, 'MAG_TREICHVILLE', 'Magasin Treichville', true)`,
      [magLocId]
    );
    console.log('Magasin Treichville created');
